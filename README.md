...
- **Inclusion par défaut des configurations, agents et compétences (dossier `.copilot`) prêts à l'emploi.**
...
### 2. Authentification
Une fois l'alias configuré, exécutez simplement la commande `copilot` (sans arguments ou avec `/login`) pour vous connecter. La configuration sera sauvegardée localement dans votre répertoire `~/.config/github-copilot` grâce aux volumes Docker. Le dossier `~/.copilot` (contenant les agents et configs) sera automatiquement initialisé au premier lancement s'il est vide.
...
**[FR] Création d'un GitHub PAT pour Copilot**

* Allez dans [GitHub Personal Access Tokens](https://github.com/settings/personal-access-tokens/new)
* Créez un token d'accès précis (fine-grained PAT)
* Sous "Permissions", cliquez sur "Add permissions"
* Sélectionnez "Copilot Requests" (read and write)
* Générez et sauvegardez votre token

Il faut aussi rajouter ces variables dans votre fichier `~/.bash_profile` / You also need to add these variables to your `~/.bash_profile`:
